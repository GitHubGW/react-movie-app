...
위에서 만든 App함수를 외부에서 사용하려면 export해줘야 한다.
export default App2;
*/
...
// 클래스 컴포넌트인 App2을 만들고 extends 키워드를 통해 React.Component를 확장했다.
// React.Component를 extends해주게 되면 App클래스는 React.Component(리액트 컴포넌트)가 되고 리액트 컴포넌트가 가지고 있는 모든 특징과 기능들을 가져올 수 있다.
// 클래스 컴포넌트인 App2는 위에 function App2(){}처럼 return을 가지고 있지 않다. 왜냐하면 함수가 아니기 때문이다.
...
class App2 extends React.Component {
  ...
  add = () => {
    ...
    // 랜더함수가 재 실행이 되지 않으면 변경된 state값이 화면상에 보이지 않음 (실제로 state값은 바뀐건 맞음)
    // 중요! 그래서 우리는 아래와 같이 setState()메서드를 통해 state값을 변경함으로써 랜더 함수를 재 실행시켜줄 수 있다.
    // 즉, state의 값을 변경할 때마다 리액트가 랜더 함수를 refresh(재 실행)시켜주도록 해주는 것이다.
    // this.state.count = 1;

    /*
    // setState()함수를 이용해서 state의 값을 재 설정해준다. (랜더링할 state는 객체이기 때문에 this.setState({})형태로 {}로 묶어줬다.)
    // 리액트는 setState()를 쓰게 되면 자동으로 재 랜더링해준다. (단, 리액트는 바뀐 부분만 재 랜더링한다.)
    this.setState({
  ...
  // componentDidUpdate는 컴포넌트가 업데이트 되었을 때 (예를들면 state나 props값 등이 업데이트 되었을 떄) 실행된다. (render()가 끝난 후에 실행된다.)
  componentDidUpdate() {
